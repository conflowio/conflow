// Code generated by Conflow. DO NOT EDIT.

package directives

import (
	"fmt"

	"github.com/conflowio/conflow/pkg/conflow"
	"github.com/conflowio/conflow/pkg/conflow/annotations"
	"github.com/conflowio/conflow/pkg/conflow/bind"
	"github.com/conflowio/conflow/pkg/schema"
)

func init() {
	schema.Register(&schema.Object{
		Metadata: schema.Metadata{
			Annotations: map[string]string{
				annotations.Type: "directive",
			},
			ID: "github.com/conflowio/conflow/pkg/schema/directives.MinProperties",
		},
		Properties: map[string]schema.Schema{
			"id": &schema.String{
				Metadata: schema.Metadata{
					Annotations: map[string]string{
						annotations.ID: "true",
					},
					ReadOnly: true,
				},
				Format: "conflow.ID",
			},
			"value": &schema.Integer{
				Metadata: schema.Metadata{
					Annotations: map[string]string{
						annotations.Value: "true",
					},
				},
			},
		},
		Required: []string{"value"},
	})
}

// NewMinPropertiesWithDefaults creates a new MinProperties instance with default values
func NewMinPropertiesWithDefaults() *MinProperties {
	b := &MinProperties{}
	return b
}

// MinPropertiesInterpreter is the Conflow interpreter for the MinProperties block
type MinPropertiesInterpreter struct {
}

func (i MinPropertiesInterpreter) Schema() schema.Schema {
	s, _ := schema.Get("github.com/conflowio/conflow/pkg/schema/directives.MinProperties")
	return s
}

// Create creates a new MinProperties block
func (i MinPropertiesInterpreter) CreateBlock(id conflow.ID, blockCtx *conflow.BlockContext) conflow.Block {
	b := NewMinPropertiesWithDefaults()
	b.id = id
	return b
}

// ValueParamName returns the name of the parameter marked as value field, if there is one set
func (i MinPropertiesInterpreter) ValueParamName() conflow.ID {
	return "value"
}

// ParseContext returns with the parse context for the block
func (i MinPropertiesInterpreter) ParseContext(ctx *conflow.ParseContext) *conflow.ParseContext {
	var nilBlock *MinProperties
	if b, ok := conflow.Block(nilBlock).(conflow.ParseContextOverrider); ok {
		return ctx.New(b.ParseContextOverride())
	}

	return ctx
}

func (i MinPropertiesInterpreter) Param(b conflow.Block, name conflow.ID) interface{} {
	switch name {
	case "id":
		return b.(*MinProperties).id
	case "value":
		return b.(*MinProperties).value
	default:
		panic(fmt.Errorf("unexpected parameter %q in MinProperties", name))
	}
}

func (i MinPropertiesInterpreter) SetParam(block conflow.Block, name conflow.ID, value interface{}) error {
	b := block.(*MinProperties)
	switch name {
	case "value":
		propSchema, _ := i.Schema().(*schema.Object).PropertyByParameterName("value")
		bound, err := bind.BindValue(propSchema, value)
		if err != nil {
			return err
		}
		b.value = schema.Value[int64](bound)
	}
	return nil
}

func (i MinPropertiesInterpreter) SetBlock(block conflow.Block, name conflow.ID, key string, value interface{}) error {
	return nil
}
